...
public class CustomMetrics {
  ...
  /**
   * Calculate value for the software reliability growth model predictive ability metric. This
   * metric describes how long it takes for the model predictions to converge within 10 percent of
   * the final observed issue amount value.
   *
  ...
  public static Double calculateModelPredictiveAbility(
      List<Pair<Integer, Integer>> observedData, List<Pair<Integer, Integer>> modelPredictedData) {
    ...
    Pair<Integer, Integer> cumulativeResultWithinBounds =
        modelPredictedData.get(modelPredictedData.size() - 1);
  ...
  public static Double calculateResidualStandardError(
    ...
    BigInteger squareSumOfResiduals =
        calculateSumOfSquaresOfDataPointValues(
            calculateResidualsForDataPoints(observedData, modelPredictedData));

    return Math.sqrt(
        squareSumOfResiduals.divide(BigInteger.valueOf(observedData.size())).doubleValue());
  }
  ...
  private static List<Pair<Integer, Integer>> calculateResidualsForDataPoints(
    ...
    for (int i = 0; i < observedData.size(); i++) {
      ...
      Pair<Integer, Integer> residual =
          Pair.create(observed.getFirst(), observed.getSecond() - predicted.getSecond());
      result.add(residual);
    }
    return result;
  }
