...
public class GitHubDataProviderTest {
  ...
  @InjectMocks
  private GitHubGeneralIssueDataProvider provider =
      new GitHubGeneralIssueDataProvider(new GitHubClient());
  ...
  public void testGetIssuesByUrl() throws IOException {
    ...
    when(issueService.getIssues("user", "repository", filter))
        .thenReturn(Arrays.asList(new Issue().setCreatedAt(new Date())));
    filter = new HashMap<>();
