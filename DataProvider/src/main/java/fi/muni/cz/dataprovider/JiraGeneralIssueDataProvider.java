...
public class JiraGeneralIssueDataProvider implements GeneralIssueDataProvider {
  ...
  public List<GeneralIssue> getIssuesByUrl(String url) {
    try {
      ...
      return readJiraIssuesFromCsvFile(url).stream()
  ...
  private List<JiraIssue> readJiraIssuesFromCsvFile(String filePath)
      throws IOException, CsvValidationException {
    return readDataFromCsvFile(filePath).stream()
        .map(this::mapRawDataToJiraIssue)
        .collect(Collectors.toList());
  }

  private List<Map<String, String>> readDataFromCsvFile(String filePath)
      throws IOException, CsvValidationException {

    CSVReaderHeaderAware csvReader =
        new CSVReaderHeaderAwareBuilder(new FileReader(filePath))
  ...
  private Date parseDateFromString(String string) {
    DateTimeFormatter formatter =
        DateTimeFormatter.ofPattern(ACCEPTED_DATE_FORMAT).withLocale(Locale.ENGLISH);
