...
public class GitHubRepositoryInformationDataProvider implements RepositoryInformationDataProvider {
  ...
  public GitHubRepositoryInformationDataProvider(GitHubClient client) {
    ...
    try {
      ...
      if (authProvider.getOAuthToken() == null || authProvider.getOAuthToken().isEmpty()) {
        github =
            new GitHubBuilder()
                .withPassword(authProvider.getUserName(), authProvider.getPassword())
                .build();
      } else {
  ...
  public RepositoryInformation getRepositoryInformation(String url) {
    ...
    return getRepositoryInformationOwnerRepoName(
        parsedUrlData.getUserName(), parsedUrlData.getRepositoryName());
  }

  private RepositoryInformation getRepositoryInformationOwnerRepoName(
    ...
    try {
      ...
      repositoryInformation = dozerBeanMapper.map(repository, RepositoryInformation.class);
      repositoryInformation.setPushedAtFirst(repository.getCreatedAt());
      // for linux repository, error is thrown too many contributors
      repositoryInformation.setContributors(
          repository.getName().equals("linux")
              ? 0
              : repositoryService.getContributors(repository, false).size());
      ...
      throw new AuthenticationException(
          "Bad credentials set. " + "Wrong name or password in authentication file.", ex);
    } catch (IOException ex) {
      log(Level.SEVERE, "Error while getting repository by Owner and Repository name.", ex);
    }
  ...
  private void addReleases(String repoFullName, RepositoryInformation repositoryInformation)
      throws IOException {
    github
        .getRepository(repoFullName)
        .listReleases()
        .forEach(
            release -> {
  ...
  private void log(Level level, String message, Exception ex) {
    Logger.getLogger(GitHubGeneralIssueDataProvider.class.getName()).log(level, message, ex);
  }
}
