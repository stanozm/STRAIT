...
public class GitHubAuthenticationDataProvider {
  ...
  private void gitHubClientSetUserNameAndPassword() {
    if (userName == null || password == null) {
      throw new AuthenticationException("OAuthToken + " + "UserName or Password not set.");
    }
    if (userName.isEmpty() || password.isEmpty()) {
      throw new AuthenticationException(
          "OAuthToken or " + "UserName and Password cannot be empty.");
    }
  ...
  private void parseDataFromAuthenticationFile() {
    ...
    try (InputStream stream = new FileInputStream(AUTH_FILE_NAME)) {
      ...
      Logger.getLogger(GitHubGeneralIssueDataProvider.class.getName())
          .log(Level.SEVERE, "Error while getting data from " + AUTH_FILE_NAME, ex);
      throw new AuthenticationFileErrorException(
          "Error while getting data from " + AUTH_FILE_NAME, ex);
    }
  }
}
