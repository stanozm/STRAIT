...
public class FilterByLabelNGTest {
    ...
    public void testFilterWithNoWords() {
        filterWithNoWords.apply(listOfIssues, null);
    }
    
    @Test
    public void testFilterWithWords() {
        assertEquals(filterForWordError.apply(listOfIssues, null).size(), 1);
        assertEquals(filterForSomeWord.apply(listOfIssues, null).size(), 0);
    }
}