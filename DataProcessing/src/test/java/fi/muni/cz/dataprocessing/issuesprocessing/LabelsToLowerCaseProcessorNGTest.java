...
public class LabelsToLowerCaseProcessorNGTest {
  ...
  public void testLabelsToLowerCase() {
    assertEquals(
        processor.apply(listOfIssues, new RepositoryInformation()).get(0).getLabels().get(0),
        "bug");
    assertEquals(
        processor.apply(listOfIssues, new RepositoryInformation()).get(0).getLabels().get(1),
        "error");
    assertEquals(
        processor.apply(listOfIssues, new RepositoryInformation()).get(0).getLabels().get(2),
        "fault");
  }
}
