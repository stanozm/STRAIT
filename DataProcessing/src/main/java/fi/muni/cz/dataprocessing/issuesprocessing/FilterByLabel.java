...
public class FilterByLabel implements Filter, Serializable {
   ...
   * @param negativeMode boolean. Includes issues that match filter if false. Includes issues that
   *     do not match filter if false.
   */
  ...
  public String toString() {
    return "FilterByLabel"
        + filteringWords.stream().map(str -> str + " ").collect(Collectors.joining());
  }
}
