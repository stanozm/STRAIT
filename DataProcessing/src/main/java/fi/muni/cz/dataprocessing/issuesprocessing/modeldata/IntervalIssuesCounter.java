...
public class IntervalIssuesCounter implements IssuesCounter {
  ...
  public List<Pair<Integer, Integer>> countIssues(
      List<GeneralIssue> issues, Date startOfTesting, Date endOfTesting) {
    if (issues == null || issues.isEmpty()) {
      throw new NullPointerException("listOfIssues is null or empty.");
    }

    return sortingIssuesIntoTimePeriods(
        issues, getDateFromMidNight(startOfTesting), getDateFromMidNight(endOfTesting));
  }
  ...
  /**
   * Calculate date after <code>howManyToAdd</code> <code>typeOfTimeToAdd</code> units of time.
   *
