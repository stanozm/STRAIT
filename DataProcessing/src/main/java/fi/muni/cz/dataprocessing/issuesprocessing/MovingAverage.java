...
public class MovingAverage {
  ...
  /**
   * Calculate a simple moving average over the given data. Current element is the last element of
   * the moving average window. Float-type average is rounded to the nearest integer.
   *
  ...
  public static List<Pair<Integer, Integer>> calculateMovingAverage(
      List<Pair<Integer, Integer>> inputData, int windowSize) {
    ...
    for (int i = windowSize - 1; i < inputData.size(); i++) {
      List<Pair<Integer, Integer>> windowElements =
          paddedData.subList(i - windowSize / 2, i + windowSize / 2);
      float average =
          windowElements.stream().map(Pair::getSecond).reduce(0, Integer::sum) / (float) windowSize;
  ...
  private static List<Pair<Integer, Integer>> padDataWithEdgeValues(
      List<Pair<Integer, Integer>> data, Integer padAmount) {
