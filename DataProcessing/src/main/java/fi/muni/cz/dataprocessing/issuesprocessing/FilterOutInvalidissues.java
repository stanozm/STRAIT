...
public class FilterOutInvalidissues implements Filter {
  ...
  private static final List<String> FILTERING_WORDS = Arrays.asList("invalid", "obosolete");
  ...
  public List<GeneralIssue> apply(
    ...
    Set<GeneralIssue> filteredList =
        FILTER_BY_LABELS.apply(list, null).stream()
            .filter(
                issue ->
                    issue.getCreatedAt() == null
                        || issue.getClosedAt() == null
                        || (issue.getClosedAt().getTime() - issue.getCreatedAt().getTime()) / 1000.0
                            > 600)
