...
public class FilterFactory {
  ...
  public static List<IssueProcessingAction> getFilters(ArgsParser parser) {
    ...
    if (parser.hasOptionFilterLabels()) {
      if (parser.getOptionValuesFilterLabels() == null) {
        ...
        listOfFilters.add(
            new FilterByLabel(Arrays.asList(parser.getOptionValuesFilterLabels()), false));
      }
    }
   ...
   * @return List of dates where the first date is the date period start date and the second date is
   *     the end date.
   */
