...
public class CumulativeIssueAmountCalculationPhase implements ReliabilityAnalysisPhase {
  ...
  public ReliabilityAnalysisDto execute(ReliabilityAnalysisDto dto) {
    ...
    List<DataPointCollection> cumulativeAmounts =
        issueCollections.stream()
            .map(
                issuesCollection ->
                    calculateCumulativeIssueAmounts(
                        issuesCollection,
                        dto.getTestingPeriodStartDate(),
                        dto.getTestingPeriodEndDate()))
  ...
  private DataPointCollection calculateCumulativeIssueAmounts(
      GeneralIssuesCollection issuesCollection, Date startOfTesting, Date endOfTesting) {
