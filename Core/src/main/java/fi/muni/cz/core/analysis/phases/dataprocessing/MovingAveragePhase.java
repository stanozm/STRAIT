...
public class MovingAveragePhase implements ReliabilityAnalysisPhase {
  ...
  public ReliabilityAnalysisDto execute(ReliabilityAnalysisDto dto) {
    ...
    List<DataPointCollection> averagedPoints =
        dataPointCollections.stream()
            .map(
                cumulativePoints ->
                    calculateMovingAverage(cumulativePoints, getAverageWindowSize(dto)))
  ...
  private DataPointCollection calculateMovingAverage(
      DataPointCollection dataPointCollection, Integer windowSize) {
